...
document.addEventListener('keydown', (e) => {
    ...
    switch (e.key) {
        ...
        // Number keys jump to chapter dividers (1-9)
        case '1':
        case '2':
        case '3':
        case '4':
...
function initMermaid() {
    ...
    // Run explicitly. startOnLoad can miss its window when initialize() is
    // called after DOMContentLoaded (this script is inlined at end of body).
    try {
        mermaid.run({ querySelector: '.mermaid' }).then(() => {
            // After render, force every produced SVG to fill its container,
            // then enable zoom + pan on each diagram.
